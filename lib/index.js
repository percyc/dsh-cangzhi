...
async function apply(ctx, config) {
	...
	const propagatePolicyToChildren = (parentId, enabled) => {
		...
		for (const candidate of list) {
			...
			applySessionPolicy(candidate, enabled);
		}
	};
	ctx.on("agent/created", ({ agent }) => {
		const parentId = agent.session.header.parentSession;
		const inheritedFromParent = parentId !== void 0 && desiredPolicies.get(parentId) === false;
