	...
	factory: (require) => {
		...
		*/
		...
		function KnowledgeWorkbench({ useCangzhiConsole, openKnowledge, closeKnowledge, openConsole }) {
			...
			const loadDatasetRows = async (datasetId, offset, limit, evidence) => {
				const requestId = ++tableRequestRef.current;
				setBusy(true);
				const rowsResponse = await fetch(`${API}/datasets/${datasetId}/rows?offset=${offset}&limit=${limit}`, {
					credentials: "include",
					cache: "no-store"
				});
			...
			const previewEvidence = async (evidence) => {
				...
				if (!response.ok) {
					setPreviewState(await errorMessage(response, "原始证据读取失败"));
					setBusy(false);
					return;
				}
				...
				if (evidence.datasetId !== null && evidence.sourceRows.length > 0) {
					...
					if (!rowsResponse.ok) {
						setPreviewState(await errorMessage(rowsResponse, "贡献原始行读取失败"));
						setBusy(false);
						return;
					}
					setEvidenceRows(await rowsResponse.json());
					...
					if (await loadDatasetRows(datasetId, 0, PREVIEW_PAGE_SIZE, {
