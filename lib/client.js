window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var Cangzhi_module_css_default = {
			"answerEvidenceButton": "sRLa1a_answerEvidenceButton",
			"answerEvidenceNode": "sRLa1a_answerEvidenceNode",
			"answerPreview": "sRLa1a_answerPreview",
			"assistantEvidenceAction": "sRLa1a_assistantEvidenceAction",
			"brand": "sRLa1a_brand",
			"cangzhiMark": "sRLa1a_cangzhiMark",
			"cangzhiMarkdown": "sRLa1a_cangzhiMarkdown",
			...
			"datasetEvidenceSummary": "sRLa1a_datasetEvidenceSummary",
			...
			"evidenceAssetActions": "sRLa1a_evidenceAssetActions",
			"evidenceEmpty": "sRLa1a_evidenceEmpty",
			"evidenceHeading": "sRLa1a_evidenceHeading",
			"evidencePreview": "sRLa1a_evidencePreview",
			"exactEvidenceMeta": "sRLa1a_exactEvidenceMeta",
			"exactEvidencePath": "sRLa1a_exactEvidencePath",
			"exactEvidencePreview": "sRLa1a_exactEvidencePreview",
			...
			"knowledgeToolButton": "sRLa1a_knowledgeToolButton",
			...
			"markdownFormatBar": "sRLa1a_markdownFormatBar",
			"markdownFormatButton": "sRLa1a_markdownFormatButton",
			...
			"tableFill": "sRLa1a_tableFill",
			"tablePagination": "sRLa1a_tablePagination",
			"tablePreview": "sRLa1a_tablePreview",
			"tableScroll": "sRLa1a_tableScroll",
		...
		//#endregion
		//#region src/client/lib/latest-request.mjs
		/**
		...
		*/
		function createLatestRequest() {
			...
			function begin() {
				...
				return {
					...
					assertCurrent: () => {
						if (current !== ticket) throw new DOMException("request superseded by a newer request", "AbortError");
					}
				};
			}
			return {
		...
		//#endregion
		//#region src/client/lib/draft-prompt.mjs
		/** Prepare a plain-text append without rewriting structured editor content. */
		function prepareKnowledgeDraft(input, prompt) {
			if (!input || input.phase !== "plain" || input.occurrences?.length > 0) return { error: "草稿包含引用卡片或正在提交，已保留原内容。请先完成编辑，再准备资料提问。" };
		...
		const WORKBENCH_SIZE_MAX = 1200;
		const WORKBENCH_SIZE_TABLE = Object.freeze({
			narrow: 480,
			standard: 640,
			wide: 960
		});
		const WORKBENCH_SIZES = Object.freeze([
			"narrow",
			"standard",
			"wide"
		]);
		...
		new Set(WORKBENCH_SIZES);
		function resolveWorkbenchMaxWidth(viewportWidth) {
			if (!Number.isFinite(viewportWidth)) return WORKBENCH_SIZE_MAX;
			return Math.max(420, Math.min(WORKBENCH_SIZE_MAX, Math.floor(viewportWidth - 360)));
		}
		function clampWorkbenchWidth(value, viewportWidth = Number.POSITIVE_INFINITY) {
			if (!Number.isFinite(value)) return WORKBENCH_SIZE_TABLE.standard;
			return Math.min(resolveWorkbenchMaxWidth(viewportWidth), Math.max(420, Math.round(value)));
		}
		function resizeWorkbenchWithKey(width, key, viewportWidth = Number.POSITIVE_INFINITY) {
			if (key === "Home") return 420;
		...
		function readWorkbenchWidthFromStorage(storage, viewportWidth = Number.POSITIVE_INFINITY) {
			const reader = makeStorageLike(storage.getItem.bind(storage));
			const saved = Number(reader.getItem("cangzhi-workbench-width"));
			if (!Number.isFinite(saved) || saved < 420 || saved > 1200) return WORKBENCH_SIZE_TABLE.standard;
			return clampWorkbenchWidth(saved, viewportWidth);
		}
		//#endregion
		//#region src/client/lib/evidence.mjs
		...
		function evidenceLinkFromObject(value) {
			...
			const nestedChunk = typeof value.chunk === "object" && value.chunk !== null && !Array.isArray(value.chunk) ? value.chunk : void 0;
			const chunkId = idNumber(value.chunk_id ?? value.chunkId ?? nestedChunk?.id);
			const chunkType = optionalString(value.chunk_type ?? value.chunkType) ?? optionalString(nestedChunk?.type) ?? optionalString(nestedChunk?.chunk_type);
			const datasetId = idNumber(value.dataset_id ?? value.datasetId);
			if (chunkId === null && datasetId === null) return null;
			const title = optionalString(value.title) ?? optionalString(value.document_title) ?? `资料 #${String(documentId)}`;
			const snippet = optionalString(value.snippet) ?? optionalString(value.context_markdown) ?? optionalString(value.summary);
			const queryPlan = typeof value.query_plan === "object" && value.query_plan !== null ? value.query_plan : void 0;
			return {
				documentId,
				documentVersionId,
				chunkId,
				...
				evidenceType: optionalString(value.evidence_type ?? value.evidenceType) ?? (datasetId === null ? "document" : "dataset"),
				title,
				snippet,
		...
		function collectStructuredEvidence(value) {
			if (value === null || typeof value !== "object") return [];
			const links = [];
			collectEvidenceLinks(value, links, /* @__PURE__ */ new Set());
			return links;
		}
		function evidenceKey(link) {
			if (link.chunkId !== null && link.chunkId !== void 0) return `chunk:${link.chunkId}:v${link.documentVersionId ?? ""}`;
			return `dataset:${link.datasetId ?? ""}:v${link.documentVersionId ?? ""}:a${link.artifactVersion ?? ""}:r${(link.sourceRows ?? []).join(",")}`;
		}
		function dedupeEvidenceLinks(links, limit) {
			const out = [];
		...
		*/
		function isCatalogHint(link) {
			if (link === null || typeof link !== "object") return false;
			if (link.documentVersionId === null || link.documentVersionId === void 0) return false;
			if (!Array.isArray(link.sourceRows) || link.sourceRows.length !== 0) return false;
			if (link.chunkType === "dataset_catalog") return true;
			if (link.datasetId === null || link.datasetId === void 0) return false;
			if ((link.chunkId === null || link.chunkId === void 0) && (link.chunkType === null || link.chunkType === void 0)) return true;
			return false;
		}
		/**
		...
		*/
		function suppressCatalogHints(links) {
			if (!Array.isArray(links)) return [];
			const exactKeys = /* @__PURE__ */ new Set();
			const exactDocumentVersions = /* @__PURE__ */ new Set();
			for (const link of links) {
				if (link === null || typeof link !== "object") continue;
				if (link.datasetId === null || link.datasetId === void 0) continue;
				if (link.documentVersionId === null || link.documentVersionId === void 0) continue;
			...
			for (const link of links) {
				if (isCatalogHint(link)) {
					if (link.datasetId === null || link.datasetId === void 0 ? exactDocumentVersions.has(`${link.documentId}:${link.documentVersionId}`) : exactKeys.has(`${link.documentId}:${link.documentVersionId}:${link.datasetId}`)) continue;
				}
				out.push(link);
			}
			return out;
		...
		function evidenceFromToolResult(toolName, content) {
		...
		function fallbackAnswerEvidence(answer) {
			...
			return {
				...
				title: ANSWER_TITLE_PATTERN.exec(answer)?.[1]?.trim() || "数据表证据",
				snippet: void 0,
				page: null,
				headingPath: [],
				sourceRows: [],
		...
		function answerEvidence$1(answer) {
			if (typeof answer !== "string" || answer.length === 0) return null;
			const parsed = safeParseObject(answer);
			if (parsed !== null) {
				const withDataset = collectStructuredEvidence(parsed).find((link) => link.datasetId !== null);
				if (withDataset !== void 0) return withDataset;
			}
			return fallbackAnswerEvidence(answer);
		}
		function formatEvidenceLink(link) {
		...
		//#endregion
		//#region src/client/lib/markdown-preview.mjs
		/**
		...
		*/
		function byteLengthUtf8(text) {
			...
			if (textEncoderSingleton !== null) return textEncoderSingleton.encode(text).length;
			let bytes = 0;
			for (let i = 0; i < text.length; i++) {
				...
				if (code >= 55296 && code <= 56319 && i + 1 < text.length) {
					...
					if (trail >= 56320 && trail <= 57343) {
						code = 65536 + (code - 55296 << 10) + (trail - 56320);
						i += 1;
						bytes += 4;
						continue;
					}
		...
		if (typeof TextEncoder !== "undefined") textEncoderSingleton = new TextEncoder();
		/** Maximum number of bytes we will hand to MarkdownText from one evidence payload. */
		const EVIDENCE_MARKDOWN_BYTE_LIMIT = 64 * 1024;
		/**
		...
		*/
		function normalizeEvidenceMarkdown(value) {
		...
		*/
		function shouldRenderFormattedMarkdown(text, options = {}) {
			if (typeof text !== "string") return false;
			if (text.trim().length === 0) return false;
			const limit = Number.isFinite(options.byteLimit) && options.byteLimit > 0 ? Math.floor(options.byteLimit) : EVIDENCE_MARKDOWN_BYTE_LIMIT;
		...
		*/
		function truncateEvidenceMarkdown(text, options = {}) {
			...
			if (end > 0 && end < text.length) {
				...
				if (leading >= 55296 && leading <= 56319 && trailing >= 56320 && trailing <= 57343) end -= 1;
			}
			const newline = text.lastIndexOf("\n", end);
			if (newline > end * .5) end = newline;
			return `${text.slice(0, end).trimEnd()}${markerBlock}`;
		}
		/**
		...
		*/
		...
		//#endregion
		//#region src/client/lib/refresh-policy.mjs
		/**
		...
		*/
		const REFRESH_INTERVAL_ACTIVE_MS = 3e3;
		const REFRESH_INTERVAL_IDLE_MS = 15e3;
		const REFRESH_INTERVAL_HIDDEN_MS = 6e4;
		const REFRESH_FAILURE_BASE_MS = 5e3;
		const REFRESH_FAILURE_MAX_MS = 3e4;
		/**
		...
		*/
		...
		function resolveRefreshInterval(input) {
			...
			if (input.hasFailure === true) {
				...
				const backoff = REFRESH_FAILURE_BASE_MS * Math.pow(2, Math.min(streak, 4));
		...
		*/
		function shouldLightweightPoll(input) {
			if (!input || typeof input !== "object") return false;
		...
		*/
		function mergeSystemStatus(prev, next) {
			if (next === null || next === void 0) return prev;
			if (prev === null || prev === void 0) return next;
			return {
				...prev,
				...next,
				processing: {
		...
		*/
		function hasMoreOverviewPages({ offset, limit, total }) {
		...
		*/
		function buildOverviewParams({ workspace, limit = 200, offset = 0 } = {}) {
			const params = new URLSearchParams();
			params.set("limit", String(Math.max(1, limit)));
			params.set("offset", String(Math.max(0, offset)));
			params.set("include_processing", "true");
			if (typeof workspace === "string" && workspace.length > 0) params.set("workspace", workspace);
			return params;
		}
		const REFRESH_CONSTANTS = Object.freeze({
			...
			REFRESH_FAILURE_MAX_ATTEMPTS: 4,
			OVERVIEW_DEFAULT_LIMIT: 200
		});
		//#endregion
		//#region src/client/lib/preview-routing.mjs
		...
		function hasTableLocation(value) {
			...
			return Boolean(location.sheet_name) || Array.isArray(location.table_ranges) && location.table_ranges.length > 0 || Array.isArray(location.column_names) && location.column_names.length > 0;
		}
		/**
		...
		*/
		function classifyEvidencePreview(context, rows) {
			...
			if (evidenceType === "dataset" || hasRows || record(payload.dataset) !== null || hasTableLocation(payload.table_location) || [
				"xlsx",
				"xls",
				"csv",
				"spreadsheet",
				"database_table"
			].includes(documentType)) return "dataset";
			if (evidenceType === "markdown" || documentType === "markdown" || documentType === "note") return "markdown";
			if (evidenceType === "pdf_word" || evidenceType === "document" || [
				"pdf",
				"doc",
				"docx"
			].includes(documentType)) return "document";
		...
		function classifyDocumentPreview({ contentKind, sourceType, documentType, title } = {}) {
			...
			if (kind === "dataset" || [
				...
				"csv",
				"spreadsheet"
			].includes(type)) return "dataset";
			if (source === "note" || [
				"markdown",
				"md",
				"note"
			].includes(type) || /\.md$/u.test(name)) return "markdown";
			if ([
				"pdf",
				"doc",
				"docx"
			].includes(type) || /\.(pdf|docx?)$/u.test(name)) return "binary-document";
			return "text";
		}
		/**
		...
		*/
		function resolveEvidenceDatasetId(context, linkedDatasetId) {
			const contextId = positiveId(record((record(context) ?? {}).dataset)?.dataset_id);
		...
		function createConsoleFace(ctx) {
			...
			return {
				...
				openKnowledge: (tab = snapshot.knowledgeTab) => {
					publish({
						open: false,
						knowledgeOpen: true,
						knowledgeTab: tab
		...
		async function syncModelWorkspace(slug, sessionId) {
			if (sessionId === void 0) {
				window.dispatchEvent(new CustomEvent("cangzhi-workspace-changed", { detail: {
					slug,
					sessionId: void 0,
					bound: false
				} }));
		...
		*/
		function useKnowledgeSession(sessionId) {
			...
			(0, react.useEffect)(() => {
				...
				let alive = true;
				fetch(`/_cangzhi-plugin/session-policy?sessionId=${encodeURIComponent(sessionId)}`, { cache: "no-store" }).then(async (response) => {
		...
		* bound to the current Session input machine. */
		function KnowledgeToolButton({ sessionId, useCangzhiConsole, openKnowledge, inputActions, useInput, t }) {
			const inputSnapshot = useInput((value) => value);
			const inputSnapshotRef = (0, react.useRef)(inputSnapshot);
			...
			const [workspaceLabel, setWorkspaceLabel] = (0, react.useState)(t("popoverWorkspace"));
			const load = async () => {
				const [statusResponse, workspaceResponse] = await Promise.all([fetch("/_cangzhi-plugin/status", { cache: "no-store" }), fetch(`/_cangzhi-plugin/workspace?sessionId=${encodeURIComponent(sessionId)}`, { cache: "no-store" })]);
				if (statusResponse.ok) {
					const status = await statusResponse.json();
					setConfigured(Boolean(status.mcpConfigured));
				}
				if (!workspaceResponse.ok) return;
				const scoped = await workspaceResponse.json();
				if (!scoped.workspace) return;
				const workspacesResponse = await fetch(`${API}/workspaces`, {
					credentials: "include",
					cache: "no-store"
				});
				setWorkspaceLabel((workspacesResponse.ok ? await workspacesResponse.json() : []).find((item) => item.slug === scoped.workspace)?.name ?? scoped.workspace);
			};
			(0, react.useEffect)(() => {
				load().catch(() => {
					setConfigured(false);
				...
				const useDocument = (event) => {
					...
					window.dispatchEvent(new CustomEvent("cangzhi-draft-feedback", { detail: {
						sessionId,
						message: prepared.error ?? "已保留原草稿并准备资料提示，请检查输入框后发送；本操作不会自动开启藏知。"
					} }));
				};
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
				type: "button",
				className: Cangzhi_module_css_default.knowledgeToolButton,
				...
				children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: "藏知" }),
		...
		*/
		async function loadOverviewAll({ workspace, limit = REFRESH_CONSTANTS.OVERVIEW_DEFAULT_LIMIT, signal }) {
			const collected = [];
			let total = 0;
			let offset = 0;
			while (true) {
			...
			return {
				items: collected,
				total: total === 0 ? collected.length : total
			};
		}
		...
		function NativeWorkspace() {
			...
			(0, react.useEffect)(() => {
				...
				const latestProcessing = { current: {
					active: 0,
					waiting: 0
				} };
				const clearTimer = () => {
				...
				const lightweightRefresh = async () => {
					...
					try {
						const [overviewPage, systemResponse] = await Promise.all([loadOverviewAll({
						...
						if (nextSystem.processing.failed_by_workspace !== void 0) {
						...
						lifecycle.failureStreak = Math.min(lifecycle.failureStreak + 1, 4);
					} finally {
		...
		function evidenceAssetUrl(value) {
			if (!value) return null;
			if (value.startsWith("/api/")) return `${API}${value.slice(4)}`;
			return value;
		}
		function WorkbenchMarkdownPreview({ markdown }) {
			const normalizedMarkdown = (0, react.useMemo)(() => normalizeEvidenceMarkdown(markdown), [markdown]);
			const markdownLabels = (0, react.useMemo)(() => buildMarkdownLabels(), []);
			const formattedMarkdown = (0, react.useMemo)(() => truncateEvidenceMarkdown(normalizedMarkdown), [normalizedMarkdown]);
			const canFormat = shouldRenderFormattedMarkdown(formattedMarkdown);
			const [preferredView, setPreferredView] = (0, react.useState)("formatted");
			const effectiveView = canFormat ? preferredView : "raw";
			if (!normalizedMarkdown) return null;
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [canFormat && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: Cangzhi_module_css_default.markdownFormatBar,
				role: "tablist",
				"aria-label": "Markdown 渲染模式",
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
					type: "button",
					role: "tab",
					"aria-selected": effectiveView === "formatted",
					className: Cangzhi_module_css_default.markdownFormatButton,
					"data-active": effectiveView === "formatted",
					onClick: () => setPreferredView("formatted"),
					children: "格式化"
				}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
					type: "button",
					role: "tab",
					...
					className: Cangzhi_module_css_default.markdownFormatButton,
					"data-active": effectiveView === "raw",
					onClick: () => setPreferredView("raw"),
					children: "原文"
				})]
			}), effectiveView === "formatted" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: Cangzhi_module_css_default.cangzhiMarkdown,
				"data-cangzhi-markdown": "workbench",
		...
		function EvidenceWorkbenchPreview({ context, rows }) {
			...
			const normalizedMarkdown = (0, react.useMemo)(() => normalizeEvidenceMarkdown(context.context_markdown), [context.context_markdown]);
			const previewKind = classifyEvidencePreview(context, rows);
			const paragraph = context.snippet?.trim() || normalizedMarkdown;
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: Cangzhi_module_css_default.exactEvidencePreview,
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: Cangzhi_module_css_default.exactEvidenceMeta,
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: "版本绑定证据" }), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("small", { children: [
							"document_version_id ",
							context.document_version_id,
							context.page ? ` · 第 ${context.page} 页` : ""
						] })]
					}),
					context.heading_path && context.heading_path.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
						className: Cangzhi_module_css_default.exactEvidencePath,
						children: context.heading_path.join(" / ")
					}),
					previewKind === "markdown" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(WorkbenchMarkdownPreview, { markdown: normalizedMarkdown }),
					previewKind === "document" && paragraph && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("blockquote", { children: paragraph }),
					previewKind === "dataset" && rows && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("p", { children: [
							...
							" / ",
							rows.requested ?? rows.rows?.length ?? 0,
							" 行",
							rows.truncated ? "（受控截取）" : ""
						] }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							className: Cangzhi_module_css_default.tableScroll,
							children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("table", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("thead", { children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("tr", { children: columns.map((column) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("th", { children: column === "row_number" ? "原始行号" : column }, column)) }) }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("tbody", { children: (rows.rows ?? []).map((row, index) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("tr", { children: columns.map((column) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("td", { children: formatPreviewValue(row[column]) }, column)) }, String(row.row_number ?? index))) })] })
						})]
					}),
					previewKind === "dataset" && !rows && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: Cangzhi_module_css_default.datasetEvidenceSummary,
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", { children: context.dataset?.name === void 0 ? "结构化数据表" : String(context.dataset.name) }),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", { children: context.snippet || "这条证据已定位到数据集，但本轮工具结果没有携带贡献行。" }),
							Array.isArray(context.table_location?.column_names) && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("small", { children: ["字段：", context.table_location.column_names.map(String).join("、")] })
						]
					}),
					previewKind === "empty" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: Cangzhi_module_css_default.previewPlaceholder,
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: "▤" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", { children: "证据元数据已核验，但没有可显示的正文片段。" })]
					}),
					(preview || original) && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: Cangzhi_module_css_default.evidenceAssetActions,
						children: [preview && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							type: "button",
							onClick: () => window.open(preview, "_blank", "noopener,noreferrer"),
							children: "打开版本预览"
						}), original && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							type: "button",
		...
		function openDocumentInWorkbench(id, title, datasetId) {
		...
		function cangzhiEvidenceLocation(context) {
		...
		const cangzhiEvidenceDefinition = {
			...
			match: (event) => {
				...
				if (event.type === "tool/call" || event.type === "tool/result" || event.type === "turn/end") return {
					id: String(event.data.turn),
					role: "update"
				};
				return null;
			...
			update: (context, match) => {
				...
				if (toolName === void 0) return context.state;
				const found = evidenceFromToolResult(toolName, result.content);
				if (found.length === 0) return context.state;
				return {
					...context.state,
					evidence: dedupeEvidenceLinks([...context.state.evidence, ...found])
				};
			},
			...
			buildViewNode: (context) => {
				...
				if (state === void 0 || state.endSeq === void 0 || state.evidence.length === 0 || location === void 0) return null;
				const evidence = suppressCatalogHints(state.evidence);
		...
		function answerEvidence(answer) {
			if (typeof answer !== "string" || answer.length === 0) return null;
			return answerEvidence$1(answer);
		}
		function openEvidenceInWorkbench(evidence) {
			window.dispatchEvent(new CustomEvent("cangzhi-open-evidence", { detail: evidence }));
		}
		function CangzhiEvidenceNode({ node }) {
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
				className: Cangzhi_module_css_default.answerEvidenceNode,
				"aria-label": `回答证据 ${node.data.evidence.length} 条`,
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("header", { children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: "▤" }),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", { children: "回答证据" }),
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("small", { children: [node.data.evidence.length, " 条 · 来自本轮藏知工具结果"] })
				] }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", { children: node.data.evidence.map((evidence, index) => {
					...
					return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("article", { children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", { children: evidence.title }),
							evidence.snippet && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", { children: evidence.snippet.replace(/\s+/g, " ").slice(0, 220) }),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("small", { children: formatEvidenceLink(evidence) })
						] }),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							type: "button",
							disabled: !exact,
							title: exact ? "在右侧查看生成答案时使用的原始证据" : "这条旧记录缺少版本信息",
							onClick: () => openEvidenceInWorkbench(evidence),
							children: exact ? "查看原始证据" : "缺少版本信息"
						})
					] }, `${evidence.chunkId ?? ""}:${evidence.datasetId ?? ""}:${evidence.documentVersionId ?? ""}:${index}`);
				}) })]
			});
		}
		function KnowledgeWorkbench(props) {
			const sessionId = (0, react.useSyncExternalStore)(props.subscribeSession, props.currentSessionId);
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(SessionKnowledgeWorkbench, { ...props }, sessionId ?? "new-conversation");
		}
		function SessionKnowledgeWorkbench({ useCangzhiConsole, currentSessionId, subscribeSession, openKnowledge, closeKnowledge, openConsole, settingsScope, t }) {
			const state = useCangzhiConsole((value) => value);
			const sessionId = (0, react.useSyncExternalStore)(subscribeSession, currentSessionId);
			...
			const searchRequestRef = (0, react.useRef)(0);
			...
			const [evidenceContext, setEvidenceContext] = (0, react.useState)(null);
			const [evidenceRows, setEvidenceRows] = (0, react.useState)(null);
			const [previewState, setPreviewState] = (0, react.useState)("选择资料后可在这里预览原文");
			const [busy, setBusy] = (0, react.useState)(false);
			const [notice, setNotice] = (0, react.useState)("");
			const [width, setWidth] = (0, react.useState)(() => readWorkbenchWidthFromStorage(window.localStorage, window.innerWidth));
			const uploadInput = (0, react.useRef)(null);
			const resizeStart = (0, react.useRef)({
				x: 0,
				width: WORKBENCH_SIZE_TABLE.standard
			});
			const widthRef = (0, react.useRef)(width);
			const workspaceSlugRef = (0, react.useRef)(null);
			const loadRequestRef = (0, react.useRef)(0);
			const tableRequestRef = (0, react.useRef)(0);
			const previewRequests = (0, react.useRef)(createLatestRequest());
			const previewWorkspaces = (0, react.useRef)(/* @__PURE__ */ new WeakMap());
			const fetchPreview = async (ticket, input, init = {}) => {
				...
				if (!scope) {
					scope = (async () => {
						...
						const response = await fetch(`/_cangzhi-plugin/workspace?sessionId=${encodeURIComponent(sessionId)}`, {
							cache: "no-store",
			...
			const load = async () => {
				...
				const [authResponse, modelWorkspaceResponse, pluginResponse] = await Promise.all([
					...
					sessionId === void 0 ? Promise.resolve(null) : fetch(`/_cangzhi-plugin/workspace?sessionId=${encodeURIComponent(sessionId)}`, { cache: "no-store" }),
				...
				const [documentsResponse, workspaceResponse, workspacesResponse] = await Promise.all([
					workspaceFetch(`${API}/documents/overview?limit=60&offset=0&include_processing=true`, {
						credentials: "include",
						cache: "no-store"
					}),
					workspaceFetch(`${API}/workspaces/current`, {
						credentials: "include",
						cache: "no-store"
					}),
					fetch(`${API}/workspaces`, {
				...
				setWorkspaces(workspacesResponse.ok ? await workspacesResponse.json() : [nextWorkspace]);
				const items = await documentsResponse.json();
				if (requestId !== loadRequestRef.current) return;
			...
			const provisionConnection = async () => {
				const tokenResponse = await fetch(`${API}/access-tokens`, {
					method: "POST",
					credentials: "include",
					headers: { "content-type": "application/json" },
			...
			const login = async (event) => {
				...
				try {
					...
					setNotice(caught instanceof Error ? caught.message : "登录失败，请重试");
				} finally {
					setBusy(false);
				}
			};
			const connect = async () => {
				if (busy) return;
				setBusy(true);
				setNotice("正在连接藏知对话工具…");
				try {
					await provisionConnection();
					setNotice("藏知对话工具已连接");
					await load();
				} catch (caught) {
					setNotice(caught instanceof Error ? caught.message : "连接失败，请重试");
				} finally {
					setBusy(false);
				}
			};
			const disconnect = async () => {
				...
				try {
					...
					setPlugin((value) => value === null ? null : {
						...value,
						mcpConfigured: false,
						toolCount: 0
					});
					...
					setNotice(caught instanceof Error ? caught.message : "断开失败，请重试");
				} finally {
					setBusy(false);
				}
			};
			const switchWorkspace = async (slug) => {
				...
				if (next === void 0 || next.slug === workspace?.slug || busy) return;
				setBusy(true);
				setNotice("正在切换知识空间…");
				try {
			...
			const setConversationPolicy = async (next) => {
				...
				try {
					...
					setNotice(caught instanceof Error ? caught.message : "藏知能力切换失败");
				} finally {
					setBusy(false);
				}
			};
			const search = async (event) => {
				...
				setNotice("");
				try {
					const response = await workspaceFetch(`${API}/search`, {
						method: "POST",
						credentials: "include",
						headers: { "content-type": "application/json" },
						body: JSON.stringify({
					...
					setResults(body.hits.map((hit) => ({
						id: hit.document_id,
						title: hit.title,
						source_type: hit.source_type,
						content_kind: hit.evidence_type === "dataset" ? "dataset" : void 0,
						dataset_id: idNumber(hit.dataset_id) ?? void 0,
			...
			const loadDatasetRows = async (datasetId, offset, limit, evidence, ticket = previewRequests.current.begin()) => {
				const requestId = ++tableRequestRef.current;
				setBusy(true);
				try {
					const rowsResponse = await fetchPreview(ticket, `${API}/datasets/${datasetId}/rows?offset=${offset}&limit=${limit}`, {
						credentials: "include",
						cache: "no-store",
						signal: ticket.signal
					});
			...
			const preview = async (item) => {
				...
				try {
					...
					if (classifyDocumentPreview({
						contentKind: item.content_kind,
						sourceType: item.source_type,
						documentType: item.document_type,
						title: item.title
					}) === "dataset" || item.dataset_id !== void 0) {
						const datasetsResponse = await fetchPreview(ticket, `${API}/datasets?document_id=${item.id}`, {
							credentials: "include",
							cache: "no-store",
							signal: ticket.signal
						});
						...
						ticket.assertCurrent();
						const dataset = item.dataset_id === void 0 ? datasets[0] : datasets.find((candidate) => candidate.id === item.dataset_id);
						if (dataset === void 0) {
							setPreviewState("数据表尚未生成可预览的数据集");
							setBusy(false);
							return;
						}
						await loadDatasetRows(dataset.id, 0, PREVIEW_PAGE_SIZE, void 0, ticket);
						return;
					}
					const detailResponse = await fetchPreview(ticket, `${API}/documents/${item.id}`, {
						credentials: "include",
						cache: "no-store",
						signal: ticket.signal
					});
					...
					if (detailResponse.ok) {
						...
						const kind = classifyDocumentPreview({
					...
					const response = await fetchPreview(ticket, `${API}/documents/${item.id}/preview`, {
						credentials: "include",
						cache: "no-store",
						signal: ticket.signal
					});
			...
			const changeTablePage = (offset, limit = previewTable?.limit ?? PREVIEW_PAGE_SIZE) => {
				if (previewTable === null || offset < 0 || offset >= previewTable.total || busy) return;
				const evidence = previewTable.evidenceVersionId === void 0 || previewTable.evidenceMode === void 0 ? void 0 : {
					versionId: previewTable.evidenceVersionId,
					mode: previewTable.evidenceMode
				};
				loadDatasetRows(previewTable.datasetId, offset, limit, evidence);
			};
			const previewEvidence = async (evidence) => {
				const ticket = previewRequests.current.begin();
				...
				try {
					...
					setSelected({
						...
						source_type: "file",
						...evidence.datasetId === null ? {} : {
							content_kind: "dataset",
							dataset_id: evidence.datasetId
						}
					});
					setTab("preview");
					setPreviewState("正在读取回答时使用的原始证据…");
					setPreviewText("");
					setPreviewTable(null);
					setEvidenceContext(null);
					setEvidenceRows(null);
					...
					const endpoint = evidence.datasetId !== null ? `${API}/v1/knowledge/evidence/by-dataset/${evidence.datasetId}?${version}${artifact}` : evidence.chunkId !== null ? `${API}/v1/knowledge/evidence/by-chunk/${evidence.chunkId}?${version}` : null;
					if (endpoint === null) {
						setPreviewState("这条记录没有可定位的片段或数据集。");
						setBusy(false);
						return;
					}
					const response = await fetchPreview(ticket, endpoint, {
						credentials: "include",
						cache: "no-store",
						signal: ticket.signal
					});
					...
					if (evidence.datasetId !== null && evidence.sourceRows.length > 0) {
						const rowsResponse = await fetchPreview(ticket, `${API}/v1/knowledge/evidence/by-dataset/${evidence.datasetId}/rows?${version}${artifact}`, {
							...
							headers: { "content-type": "application/json" },
							body: JSON.stringify({
								source_rows: evidence.sourceRows,
								columns: evidence.columns,
								limit: Math.min(200, Math.max(20, evidence.sourceRows.length))
							})
						});
						...
						if (datasetId === null) throw new Error("数据集身份缺失或与证据不一致，不能用其他数据表代替。");
						setSelected({
							id: evidence.documentId,
							title: evidence.title,
							source_type: "file",
						...
						const loaded = await loadDatasetRows(datasetId, 0, PREVIEW_PAGE_SIZE, {
			...
			(0, react.useEffect)(() => {
				const open = (event) => {
					...
					const item = documents.find((document) => document.id === detail.id) ?? {
						...
						source_type: "file",
						...idNumber(detail.datasetId) === null ? {} : {
							content_kind: "dataset",
							dataset_id: idNumber(detail.datasetId)
						}
					};
					openKnowledge();
					preview(item);
			...
			const useDocument = (item) => {
				...
				window.dispatchEvent(new CustomEvent("cangzhi-use-document", { detail: {
					sessionId,
					prompt: `请重点读取并基于藏知资料《${item.title}》（document_id: ${item.id}）回答：\n\n`
				} }));
			};
			const upload = async (files) => {
				...
				try {
					for (const [index, file] of Array.from(files).entries()) {
						...
						body.append("title", "");
						const response = await workspaceFetch(`${API}/files/upload`, {
							method: "POST",
							credentials: "include",
							body
						});
			...
			const resizeWithKeyboard = (event) => {
				...
				setWidth(next);
				try {
					window.localStorage.setItem("cangzhi-workbench-width", String(next));
				} catch {}
			};
			const resize = (event) => {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("aside", {
				...
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						...
						"aria-label": "调整藏知工作台宽度：左键加宽，右键收窄，Home 最窄，End 最宽",
						"aria-valuemin": 420,
						"aria-valuemax": WORKBENCH_SIZE_MAX,
						"aria-valuenow": width,
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("nav", {
						...
						children: [
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
								...
								children: ["本对话", pinned.length > 0 ? ` · ${pinned.length}` : ""]
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								"data-active": String(tab === "settings"),
								onClick: () => setTab("settings"),
								children: "设置"
							})
						]
					}),
					tab === "settings" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						className: Cangzhi_module_css_default.workbenchSettings,
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(CangzhiSettingsTab, {
							settingsScope,
							t
						})
					...
					}) : !auth.authenticated ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("form", {
						className: Cangzhi_module_css_default.workbenchLogin,
						onSubmit: login,
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)(CangzhiMark, { size: 44 }),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h3", { children: "登录后使用藏知" }),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", { children: "登录和连接都在当前工作台完成，不再打开额外弹窗。" }),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
								value: loginUsername,
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								disabled: busy,
								children: busy ? "正在连接…" : "登录并连接"
							})
						]
					}) : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
							className: Cangzhi_module_css_default.workbenchControls,
							"aria-label": "藏知会话控制",
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", { children: "本对话使用藏知" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("small", { children: knowledgeSession.policy === "on" ? "下一步允许调用藏知工具" : "下一步不会调用藏知工具" })] }), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								className: Cangzhi_module_css_default.workbenchPolicy,
								role: "radiogroup",
								"aria-label": "本对话是否使用藏知",
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									type: "button",
									role: "radio",
									"aria-checked": knowledgeSession.policy === "off",
									"data-active": String(knowledgeSession.policy === "off"),
									disabled: busy,
									onClick: () => void setConversationPolicy("off"),
									children: "关闭"
								}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									type: "button",
									role: "radio",
									"aria-checked": knowledgeSession.policy === "on",
									"data-active": String(knowledgeSession.policy === "on"),
									disabled: busy,
									onClick: () => void setConversationPolicy("on"),
									children: "开启"
								})]
							})] }), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", { children: "知识空间" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("select", {
								...
								type: "button",
								disabled: busy,
								onClick: () => void (plugin?.mcpConfigured ? disconnect() : connect()),
								children: plugin?.mcpConfigured ? "断开连接" : "连接工具"
							})] })]
						}),
						tab === "browse" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
							...
							children: [
									...
									children: [
										/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("form", {
											...
											children: [
												...
												/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
													disabled: busy || searching || !workspace,
													children: searching ? "搜索中…" : "搜索"
												}),
												(query || submittedQuery) && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
													type: "button",
													onClick: clearSearch,
													"aria-label": "清除搜索，返回最近资料",
													children: "清除"
												})
											]
										}),
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									...
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", { children: submittedQuery ? `“${submittedQuery}”的搜索结果` : "最近资料" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
										role: "status",
										children: searching ? "搜索中，暂保留原列表…" : `${visible.length} 项`
									})]
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									...
									children: visible.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
										...
										children: submittedQuery ? "没有找到匹配资料，请尝试更短的关键词或清除搜索。" : "当前空间暂无资料，可以上传文件或前往知识库管理。"
						...
						tab === "preview" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								...
								children: [
									...
									selected && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
										...
										children: pinned.some((item) => item.id === selected.id) ? "再次准备提问" : "准备提问"
									})
								]
							}), evidenceContext ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(EvidenceWorkbenchPreview, {
							...
							}) : previewText ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
								className: Cangzhi_module_css_default.exactEvidencePreview,
								children: classifyDocumentPreview({
									contentKind: selected?.content_kind,
									sourceType: selected?.source_type,
									documentType: selected?.document_type,
									title: selected?.title
								}) === "markdown" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(WorkbenchMarkdownPreview, { markdown: previewText }) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("pre", {
									className: Cangzhi_module_css_default.markdownPreview,
								...
								children: [
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("p", { children: [
										previewTable.evidenceMode === "catalog" && previewTable.evidenceVersionId !== void 0 ? `版本绑定目录数据集 · version ${previewTable.evidenceVersionId} · ` : "",
										"共 ",
										previewTable.total,
										" 行，当前显示第 ",
										previewTable.offset + 1,
										"–",
										Math.min(previewTable.offset + previewTable.rows.length, previewTable.total),
										" 行"
									] }),
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
										className: Cangzhi_module_css_default.tableScroll,
										children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("table", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("thead", { children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("tr", { children: previewTable.columns.map((column) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("th", { children: column }, column)) }) }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("tbody", { children: previewTable.rows.map((row, index) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("tr", { children: previewTable.columns.map((column) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("td", { children: formatPreviewValue(row[column]) }, column)) }, String(row.row_number ?? previewTable.offset + index))) })] })
									}),
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										className: Cangzhi_module_css_default.tablePagination,
										children: [
											/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", { children: [
												"第 ",
												Math.floor(previewTable.offset / previewTable.limit) + 1,
												" / ",
												Math.max(1, Math.ceil(previewTable.total / previewTable.limit)),
												" 页"
											] }),
											/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
												disabled: busy || previewTable.offset === 0,
												onClick: () => changeTablePage(previewTable.offset - previewTable.limit),
												children: "上一页"
											}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
												disabled: busy || previewTable.offset + previewTable.rows.length >= previewTable.total,
												onClick: () => changeTablePage(previewTable.offset + previewTable.limit),
												children: "下一页"
											})] }),
											/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", { children: [
												...
												/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("select", {
													disabled: busy,
													value: previewTable.limit,
													onChange: (event) => changeTablePage(0, Number(event.target.value)),
													children: [
														/* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
															value: "25",
															children: "25"
						...
						tab === "context" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
							...
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									...
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(CangzhiMark, { size: 34 }), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", { children: "提问参考资料" }), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("small", { children: [knowledgeSession.policy === "off" ? "本对话藏知已关闭；浏览资料不会自动开启检索。" : `当前空间：${workspace?.name ?? "读取中"}。`, " 此列表仅为本次打开对话时的临时记录，不代表模型已读取资料。"] })] })]
								}),
								pinned.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									className: Cangzhi_module_css_default.contextEmpty,
									children: "尚未准备参考资料。预览资料后点击“准备提问”，检查输入框并发送后才会交给模型。"
								}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									...
									children: [
										...
										/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
											...
											onClick: () => window.dispatchEvent(new CustomEvent("cangzhi-use-document", { detail: {
												sessionId,
												prompt: `请综合以下藏知资料，归纳共同结论、分歧与依据，并逐条标注来源：${pinned.map((item) => `《${item.title}》（document_id: ${item.id}）`).join("、")}。\n\n`
											} })),
											children: "准备综合提问"
										}),
										/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
											onClick: () => window.dispatchEvent(new CustomEvent("cangzhi-use-document", { detail: {
												sessionId,
												prompt: "请核对当前问题与藏知资料中的原文，指出能够确认的事实、仍有疑问的部分，并标注来源。\n\n"
											} })),
											children: "核对事实依据"
										})
									]
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("footer", {
						...
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { "data-ok": String(Boolean(auth?.authenticated && workspace)) }),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", { children: auth?.authenticated ? workspace ? "空间已连接" : "空间未就绪" : "等待登录" }),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("small", { children: busy || searching ? "正在处理…" : `${documents.length} 份资料 · ${pinned.length} 份提问参考` })
						]
					})
				]
			});
		...
		function EvidencePreview({ tool, value }) {
			...
			if (tool === "knowledge_query_dataset") {
				const links = collectStructuredEvidence(payload).filter((link) => link.datasetId !== null);
				if (links.length === 0) return null;
				const deduped = dedupeEvidenceLinks(links, 3);
				...
				return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", { children: [
							...
							deduped.length,
							" 条来源"
						] }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("small", { children: "knowledge_query_dataset" })]
					}), deduped.map((link, index) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("article", { children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: index + 1 }),
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", { children: link.title }),
							link.snippet !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", { children: link.snippet.replace(/\s+/g, " ").slice(0, 180) }),
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							onClick: () => openDocumentInWorkbench(link.documentId, link.title, link.datasetId ?? void 0),
							children: "打开来源证据 · 右侧预览"
						})
					] }, `${link.documentId}:${link.datasetId ?? ""}:${index}`))]
				});
			}
			if (tool === "knowledge_ask" && typeof payload.answer === "string") {
				...
				const inferred = collectStructuredEvidence(payload).find((link) => link.datasetId !== null) ?? answerEvidence(payload.answer);
				return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					className: Cangzhi_module_css_default.answerPreview,
					children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", { children: payload.answer.slice(0, 520) }),
						citations.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", { children: ["引用 ", citations.length] }), citations.map((citation, index) => {
							const documentId = idNumber(citation.document_id);
							const datasetId = idNumber(citation.dataset_id);
							return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
								disabled: documentId === null,
								onClick: () => {
									if (documentId !== null) openDocumentInWorkbench(documentId, String(citation.title ?? citation.document_title ?? "知识证据"), datasetId ?? void 0);
								},
								children: [
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("b", { children: index + 1 }),
						...
						citations.length === 0 && inferred !== null && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							className: Cangzhi_module_css_default.answerEvidenceButton,
							onClick: () => openDocumentInWorkbench(inferred.documentId, inferred.title, inferred.datasetId ?? void 0),
							children: "打开数据表证据 · 右侧预览"
						})
					]
				});
			}
