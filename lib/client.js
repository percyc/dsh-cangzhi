	...
	factory: (require) => {
		...
		*/
		...
		function clampWorkbenchWidth(value) {
			...
			return Math.min(760, Math.max(360, Math.round(value)));
		}
		function resizeWorkbenchWithKey(width, key) {
			if (key === "Home") return 360;
			if (key === "End") return 760;
		...
		function readWorkbenchWidthFromStorage(storage) {
			const reader = makeStorageLike(storage.getItem.bind(storage));
			const saved = Number(reader.getItem("cangzhi-workbench-width"));
			if (!Number.isFinite(saved) || saved < 360 || saved > 760) return WORKBENCH_SIZE_TABLE.standard;
		...
		*/
		...
		function KnowledgePopover(props) {
			...
			const [sessionWorkspace, setSessionWorkspace] = (0, react.useState)(null);
			...
			(0, react.useEffect)(() => {
				...
				let alive = true;
				fetch(`/_cangzhi-plugin/workspace?sessionId=${encodeURIComponent(sessionId)}`, { cache: "no-store" }).then((response) => response.ok ? response.json() : null).then((value) => {
					if (alive) setSessionWorkspace(value);
				}).catch(() => {
					if (alive) setSessionWorkspace(null);
				});
				const update = (event) => {
					const detail = event.detail;
					if (detail?.sessionId !== void 0 && detail.sessionId !== sessionId) return;
					if (detail?.slug !== void 0) setSessionWorkspace({
						workspace: detail.slug,
						bound: detail.bound === true
					});
				};
				...
				children: [
					...
					showLogin ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("form", {
						...
						children: [
							...
							sessionId !== void 0 && sessionWorkspace !== null && !sessionWorkspace.bound && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
								className: Cangzhi_module_css_default.popoverScopeNote,
								"data-degraded": "true",
								children: t("popoverScopeDegraded")
							})
						]
					}),
		...
		function HomeIntegration({ currentSessionId, subscribeSession, openConsole, openKnowledge, t }) {
			...
			const load = async (overrideSessionId = sessionId) => {
				...
				const [workspaceResponse, workspacesResponse, modelResponse] = await Promise.all([
					...
					overrideSessionId !== void 0 ? fetch(`/_cangzhi-plugin/workspace?sessionId=${encodeURIComponent(overrideSessionId)}`, { cache: "no-store" }) : Promise.resolve(null)
				]);
				if (!workspaceResponse.ok || !workspacesResponse.ok) return;
				const [currentWorkspace, allWorkspaces] = await Promise.all([workspaceResponse.json(), workspacesResponse.json()]);
				if (requestId !== loadRequestRef.current) return;
				setWorkspaces(allWorkspaces);
				let modelWorkspace = null;
				if (modelResponse !== null) {
					const ok = modelResponse.ok ? modelResponse : null;
					const slug = (ok !== null ? await ok.json().catch(() => null) : null)?.workspace;
					if (typeof slug === "string" && slug.length > 0) modelWorkspace = allWorkspaces.find((item) => item.slug === slug) ?? {
						...currentWorkspace,
						slug
					};
				}
				if (requestId !== loadRequestRef.current) return;
				if (modelWorkspace !== null) setWorkspace(modelWorkspace);
				else {
					setWorkspace(currentWorkspace);
					if (overrideSessionId !== void 0) throw new Error("无法读取当前对话的知识空间，请重试");
				}
			};
			(0, react.useEffect)(() => {
				if (sessionId === void 0) {
		...
		function KnowledgeDock({ sessionId, openKnowledge, openConsole, inputActions, useInput, t }) {
			...
			const load = async (overrideSessionId = sessionId) => {
				...
				if (authResponse.ok) {
					...
					if (authValue.authenticated) {
						const [workspacesResponse, cookieWorkspaceResponse] = await Promise.all([fetch(`${API}/workspaces`, {
							credentials: "include",
							cache: "no-store"
						}), fetch(`${API}/workspaces/current`, {
							credentials: "include",
						...
						const all = workspacesResponse.ok ? await workspacesResponse.json() : [];
						if (requestId !== loadRequestRef.current) return;
						setWorkspaces(all);
						const cookieWorkspace = cookieWorkspaceResponse.ok ? await cookieWorkspaceResponse.json() : null;
						let resolvedFromHost = false;
						if (modelResponse !== null && modelResponse.ok) {
							const value = await modelResponse.json().catch(() => null);
							if (requestId !== loadRequestRef.current) return;
							const slug = value?.workspace;
							if (typeof slug === "string" && slug.length > 0) {
								setWorkspace(all.find((item) => item.slug === slug) ?? {
									id: 0,
									slug,
									name: slug,
									is_default: false,
		...
		function ConversationKnowledgeHeader({ sessionId, openKnowledge, t }) {
			...
			(0, react.useEffect)(() => {
				let alive = true;
				const load = async () => {
					const [statusResponse, modelResponse] = await Promise.all([fetch("/_cangzhi-plugin/status", { cache: "no-store" }), sessionId !== void 0 ? fetch(`/_cangzhi-plugin/workspace?sessionId=${encodeURIComponent(sessionId)}`, { cache: "no-store" }) : Promise.resolve(null)]);
					if (!alive) return;
					if (statusResponse.ok) {
					...
					if (modelResponse !== null && modelResponse.ok) {
						...
						if (typeof value?.workspace === "string" && value.workspace.length > 0) {
							try {
								...
								if (wsResponse.ok) {
									const list = await wsResponse.json();
									if (!alive) return;
		...
		*/
		...
		function SessionKnowledgeWorkbench({ useCangzhiConsole, currentSessionId, subscribeSession, openKnowledge, closeKnowledge, openConsole }) {
			const state = useCangzhiConsole((value) => value);
			const sessionId = (0, react.useSyncExternalStore)(subscribeSession, currentSessionId);
			...
			const [busy, setBusy] = (0, react.useState)(false);
			const [notice, setNotice] = (0, react.useState)("");
			const [width, setWidth] = (0, react.useState)(() => readWorkbenchWidthFromStorage(window.localStorage));
			...
			const load = async () => {
				...
				const [authResponse, modelWorkspaceResponse] = await Promise.all([fetch(`${API}/auth/status`, {
					credentials: "include",
					cache: "no-store"
				...
				const [documentsResponse, workspaceResponse] = await Promise.all([workspaceFetch(`${API}/documents/overview?limit=60&offset=0&include_processing=true`, {
					credentials: "include",
					cache: "no-store"
