...
当前交付：0.12.0 藏知会话入口与工作台交互收口，基于 0.11.2 的预览请求生命周期与草稿保护。输入入口迁到 DSH 原生工具行，Popover 会话控制和连接设置合并进可加宽工作台。112 项测试（无跳过）、构建、语法检查与 DSH 实机重载通过；真实浏览器视觉和交互仍待验收。
...
- 客户端控制端点显式携带 `sessionId`：`GET/POST /workspace` 与 `GET/POST /session-policy` 都以 `sessionId` 为键；workspace 查询/改动未带 `sessionId` 返回 400，仅 `processDefault:true` 允许进程级变更。`syncModelWorkspace(slug, sessionId)` 仅在持有会话时 POST；管理中心（无会话绑定）不再写任何模型工作空间。会话工具与工作台先读 `/_cangzhi-plugin/workspace?sessionId=...`、按 Host 返回的空间展示，不在加载页面时用 Cookie 自动写入会话绑定；切换会话后立刻重载，不再让一个会话的 cookie 跨写到另一个会话。
...
- 输入区工具行提供唯一的会话藏知入口，与附件、指令同排；点击直接打开工作台。
- 工作台集中提供登录、连接状态、知识空间、会话能力开关和连接设置，不再使用 Popover。
...
- 客户端会等待 Host 响应并在失败时在工作台暴露错误，避免“静默”关闭。
...
- 工作台保留连续拖拽与宽度记忆，桌面响应式范围扩大为 420–1200px；仍不提供窄/标准/宽档位和全屏按钮。
...
- 会话工具入口依赖新版 DSH 的 `conversation.input.left`；旧版缺少该槽位时仅保留侧栏全局管理入口。
...
- 活动对话的输入工具行显示紧凑藏知入口，点击直接打开工作台且不出现 Popover。
