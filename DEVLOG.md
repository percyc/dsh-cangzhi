...
## 2026-09-09：中英文项目说明与多远端发布准备

- 新增完整英文 `README.en.md`，与中文 README 互设语言入口；安装、配置、开发、知识空间边界和安全说明统一使用通用示例地址，不包含当前内网仓库、主机或本地工作目录。
- 中文 README 和部署文档同步 0.12.0 交互：从对话输入工具栏打开藏知工作台，在工作台“设置”或 DSH“设置 → 插件 → 藏知”维护同一份连接配置。
- `package.json` 发布文件清单加入 `README.en.md`。`npm pack --dry-run --json` 确认中英文 README 均进入 0.12.0 包；公开文档敏感地址扫描无命中，112 项测试及 `git diff --check` 通过。
